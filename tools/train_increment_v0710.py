...
def main():
    ...
    for tid, task in zip(task_index, task_list):
        ...
        if tid == 1:
            ...
            if tid == cfg.task.resume_by_task:
                assert getattr(cfg.task, task, None), print(f'恢复任务{task}训练，配置项未找到：cfg.task[{task}]=??')
                ...
                # 上一句可以不要，会在下面再次加载此权值参数，且同时加载epoch/lr等训练参数，而且会自动忽略此处已加载的教师模型！
                ...
                print(f'\n====== Task-{tid}({task}) Resume From Student CheckPoint: {cfg.task[task]["student_ckpt"]} '
                      f'& Teacher CheckPoint: {cfg.task[task]["teacher_ckpt"]}')
            else:
                ...
                # 此处一般无需手动配置，直接使用上一任务的model作为下一任务的教师模型。
