...
class YOLOY(SingleStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(YOLOY, self).__init__(backbone, neck, bbox_head, train_cfg, test_cfg, pretrained, init_cfg)
    ...
    def set_teacher(self, config=None, ckptfile=None, model=None, trainval='val', taskid=''):
        ...
        if config and ckptfile:
            ...
            raise NotImplementedError(f'TaskID={taskid}, 教师模型wufa设置')
        if trainval == 'val':
        ...
        print(f'TaskID={taskid}, 教师模型已设置，TrainVal：{trainval}，'
              f'权值加载：{ckptfile if not model else "byModel"}')
        ...
        with torch.no_grad():
            ...
            pred_outs = self.teacher_model.bbox_head.get_bboxes(
                *head_outs, img_metas=img_metas,
                ...
                rescale=False, with_nms=True, need_keepid=True,
                task_labels=self.LableInPCNTask)
            pred_bboxes = [result[0][:, 0:4].detach() for result in pred_outs]
            pred_logits = [result[0][:, 4:5].flatten().detach() for result in pred_outs]     # score * objectness
            pred_labels = [result[1].detach() for result in pred_outs]
            pred_scores = [result[2].detach() for result in pred_outs]      # org_score
            pred_objnes = [result[3].detach() for result in pred_outs]      # org_objnes
            pred_keepid = [result[4].detach() for result in pred_outs]
            if cat_keepid:
                pred_keepid = torch.cat(pred_keepid, dim=0)
            pred_scores = [pred_logits, pred_scores][0]
        return neck_feat, head_outs, pred_keepid, pred_labels, pred_scores, pred_bboxes, pred_objnes
    ...
    def forward_train(self,
        ...
        teacher_feats, teacher_outs, teacher_keepid, \
        teacher_labels, teacher_scores, teacher_bboxes, teacher_objnes = [None] * 7
        if self.has_teacher:
            teacher_feats, teacher_outs, teacher_keepid, teacher_labels, \
            teacher_scores, teacher_bboxes, teacher_objnes = self.out_teacher(img, img_metas, cat_keepid=True)
    ...
    def draw_boxes_on_img_v1(self, img_id=None, img_mat=None, img_info=None,
        ...
        for idx, (label, score, bbox) in enumerate(target):
            ...
            if score > 0.5:
                box_color = (255, 0, 0)     # (255, 0, 255)   # 紫红
            elif 0.3 <= score < 0.5:
            ...
            draw.line([(x_min, y_min), (x_min, y_max), (x_max, y_max),
                       (x_max, y_min), (x_min, y_min)], width=1, fill=box_color)

            # CategoryName, CategoryID, CategoryLabel
            text = ''
            # text = str(score)[:4]
            # text = self.Label2CatNameId[label][0]
            text = self.Label2CatNameId[label][0] + ['', '|' + str(score)[:3]][score > 0]   # :5
            draw.text((x_min, y_min), text, text_color, font=font)
        # image.show()
