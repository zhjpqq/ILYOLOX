...
class FocalLossCost:
    ...
    def __init__(self,
                 ...
                 gamma=2,
                 eps=1e-12,
                 activate='sigmoid',    # default value for focal loss
                 binary_input=False):
        assert activate in 'none|sigmoid|softmax'
        self.weight = weight
        self.alpha = alpha
        self.gamma = gamma
        self.eps = eps
    ...
    def _focal_loss_cost(self, cls_pred, gt_labels):
        ...
        neg_cost = -(1 - self.alpha) * (1 - cls_pred + self.eps).log() * cls_pred.pow(self.gamma)
        pos_cost = - self.alpha * (cls_pred + self.eps).log() * (1 - cls_pred).pow(self.gamma)
...
class CrossEntropyLossCost:
    ...
    def __init__(self, weight=1., use_sigmoid=True, activate='sigmoid'):
    ...
    def __call__(self, cls_pred, gt_labels):
        ...
        if self.use_sigmoid:
            ...
            raise NotImplementedError('use_sigmoid = False is not supported yet.')
